#!/usr/bin/env python3
"""Gera o PDF de um e-book HTML usando as regras de impressão do Chrome.

Uso: python3 template/gerar_pdf.py pasta/ebook.html [pasta/saida.pdf]
Sem saída informada, grava em <pasta do HTML>/output/pdf/<nome do HTML>.pdf.
"""
...
def main() -> int:
    ...
    parser.add_argument("input", type=Path, help="HTML de origem")
    parser.add_argument("output", type=Path, nargs="?", help="PDF de destino (opcional)")
    parser.add_argument("--chrome", help="Caminho para o executável do Chrome/Chromium")
    args = parser.parse_args()

    source = args.input.expanduser().resolve()
    if args.output is None:
        destination = source.parent / "output" / "pdf" / f"{source.stem}.pdf"
    else:
